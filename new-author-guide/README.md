...
Use `index.html` for the redesigned guide. The visible `Step by Step Guide` actions and the `workshops/author-guide/` and `workshops/livelabs/` compatibility routes open the live original guide, preserving a `?lab=<page-id>` target when one is supplied.
